...
require_pods() {
  ...
  if [[ -z "${pods//[[:space:]]/}" ]]; then
    echo "ERROR: no pods found for label app=${APP_LABEL} in namespace ${NAMESPACE}" >&2
    exit 1
  fi
}
...
# Always try to clear netem on exit
cleanup() {
  clear_netem || true
}
trap cleanup EXIT
...
extract_first_json_object() {
  # reads from stdin and prints compact JSON object to stdout
  "$PYTHON_BIN" -c '
import json, sys
text = sys.stdin.read()
if not text.strip():
    sys.stderr.write("EMPTY_OUTPUT\n")
    sys.exit(2)
...
sys.stderr.write("NO_JSON_OBJECT\n")
...
run_bench() {
  ...
  set +e
  local bench_output
  bench_output=$("$BENCH_BIN" -base-url "$ORDER_BASE_URL" -total "$total" -concurrency "$concurrency" 2>&1)
  local rc=$?
  set -e

  if [[ $rc -ne 0 ]]; then
    echo "ERROR: bench-runner failed (rc=$rc). Output:" >&2
    printf '%s\n' "$bench_output" >&2
    exit "$rc"
  fi
  ...
  set +e
  local bench_json
  bench_json=$(printf '%s' "$bench_output" | extract_first_json_object)
  local jrc=$?
  set -e

  if [[ $jrc -ne 0 ]]; then
    echo "ERROR: cannot extract JSON from bench-runner output. Raw output:" >&2
    printf '%s\n' "$bench_output" >&2
    exit 3
  fi
  ...
  for latency in "${LATENCIES_MS[@]}"; do
    for jitter in "${JITTERS_MS[@]}"; do
      ...
      for tx in "${TX_COUNTS[@]}"; do
        ...
        bench_json=$(run_bench "$tx" "$CONCURRENCY")
